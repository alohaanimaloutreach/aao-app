-- AAO Command Center — Initial Schema Migration
-- Phase 1: All tables, enums, RLS policies, seed data
...
-- Sequence for AAO ID generation
...
-- Function to compute food bag size from size_category
...
BEGIN
  ...
  ELSE
    -- large, xlarge, unknown all get 6lb
...
-- Partial unique index: only one active situation per animal
...
CREATE POLICY "outreach_event_volunteers_select" ON outreach_event_volunteers
  FOR SELECT TO authenticated USING (true);

CREATE POLICY "outreach_event_volunteers_insert" ON outreach_event_volunteers
  FOR INSERT TO authenticated WITH CHECK (true);

CREATE POLICY "outreach_event_volunteers_delete" ON outreach_event_volunteers
...
-- ============================================================
-- NOTE: Users are seeded via the import script using
-- supabase.auth.admin.createUser() since they need Auth entries.
-- ============================================================
