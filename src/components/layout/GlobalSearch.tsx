      ...
      <div className="relative max-w-lg w-full mx-auto mt-[10vh] bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[75vh] mx-4 sm:mx-auto">
        ...
        <div className="flex-1 overflow-y-auto" aria-live="polite">
          ...
          {animals.length > 0 && (
            <div>
              <div className="px-4 py-2 bg-sand/50">
                <p className="text-xs font-semibold text-muted uppercase tracking-wide">Animals ({animals.length})</p>
              </div>
              {animals.map((a) => (
                <button
                  key={a.id}
          ...
          {owners.length > 0 && (
            <div>
              <div className="px-4 py-2 bg-sand/50">
                <p className="text-xs font-semibold text-muted uppercase tracking-wide">People ({owners.length})</p>
              </div>
              {owners.map((o) => (
                <button
                  key={o.id}
          ...
          {locations.length > 0 && (
            <div>
              <div className="px-4 py-2 bg-sand/50">
                <p className="text-xs font-semibold text-muted uppercase tracking-wide">Locations ({locations.length})</p>
              </div>
              {locations.map((l) => (
                <button
                  key={l.id}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {l.address && <span className="text-xs text-muted">{l.address}</span>}
                  </div>
                </button>
              ))}
            </div>
        ...
        <div className="px-4 py-2 border-t border-night/5 bg-sand/30">
          <p className="text-xs text-muted text-center">Press Esc to close</p>
        </div>
      </div>
    </div>
  );
