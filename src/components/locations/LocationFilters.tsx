...
export const DEFAULT_LOCATION_FILTERS: LocationFilterState = {
  search: '',
  status: '',
  showArchived: false,
};
...
export default function LocationFilters({ filters, onChange, resultCount }: Props) {
  const [showFilters, setShowFilters] = useState(false);
  const activeFilterCount = [filters.status, filters.showArchived].filter(Boolean).length;

  function update(partial: Partial<LocationFilterState>) {
      ...
      {showFilters && (
        <div className="bg-white rounded-2xl border border-night/5 p-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm text-muted font-medium mb-1">Status</label>
              <select
          ...
          <div className="flex items-center justify-between mt-3 pt-1">
            ...
            {activeFilterCount > 0 && (
              <button onClick={() => onChange(DEFAULT_LOCATION_FILTERS)} className="text-sm text-primary hover:text-primary-hover font-medium">
                Clear all
              </button>
            )}
          </div>
