        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              {location.precise_location && <p className="text-xs text-muted/70 mt-0.5">{location.precise_location}</p>}
            </div>
          </div>
    ...
    <div className="space-y-2">
      {events.map((e) => (
        <div key={e.id} className="bg-white rounded-2xl border border-night/5 p-4">
          ...
          {e.notes && <p className="text-xs text-muted mt-2 ml-11">{e.notes}</p>}
        </div>
      ))}
    </div>
  );
