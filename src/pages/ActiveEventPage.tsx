...
export default function ActiveEventPage() {
  ...
  return (
    <div>
      {/* Test mode banner for active event */}
      {testMode && (
        <div className="test-mode-banner bg-amber-400 text-amber-900 px-4 py-2.5 -mx-4 md:-mx-6 -mt-4 md:-mt-6 mb-4 flex items-center gap-2 text-xs font-bold tracking-wide">
          <FlaskConical className="w-4 h-4" />
          TEST MODE — This event's data is being tagged for cleanup
        </div>
      )}

      {/* Event header */}
      <div className={`flex items-start justify-between gap-3 mb-4 ${testMode ? 'rounded-2xl border-2 border-amber-300 bg-amber-50/50 p-4 -mx-1' : ''}`}>
        <div>
      ...
      {/* Tabs */}
      <div className={`flex gap-1 rounded-xl p-1 mb-4 ${testMode ? 'bg-amber-200/40' : 'bg-night/5'}`}>
        <button
